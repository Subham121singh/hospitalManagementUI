...
function PatientDashboard() {
  ...
  const [patientVisibility, setPatientVisibility] = useState([]);
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((response) => {
        ...
        setPatientVisibility(new Array(response.data.length).fill(false));
      })
      .catch((error) => {
  ...
  const admitPatient = () => {
    ...
    axios
      ...
      .then((response) => {
        ...
        // Update the patients array and visibility array
        setPatients([...patients, response.data]);
        setPatientVisibility([...patientVisibility, false]);
      })
      .catch((error) => {
        console.error("Error:", error);
      });
  ...
  const handlePatientVisibility = (index) => {
    setPatientVisibility((prevVisibility) => {
      ...
      {patients.map((patient, index) => (
        ...
        >
          ...
          <div className="flex gap-5">
            <div className="">
              <button
                onClick={() => handlePatientVisibility(index)}
                className="bg-blue-600 px-5 py-3 text-white rounded"
              >
                {patientVisibility[index]
